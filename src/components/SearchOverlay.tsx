      ...
      {open && (
        <>
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            className="fixed inset-0 z-[70] bg-background/90 backdrop-blur-md" onClick={onClose} />
          ...
          >
            <div className="relative">
              ...
              <input ref={inputRef} value={query} onChange={(e) => setQuery(e.target.value)}
                ...
                className="w-full h-14 pl-12 pr-12 rounded-2xl bg-card border border-border text-foreground text-base placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/50" maxLength={100} />
            ...
            <div className="mt-3 max-h-[60vh] overflow-y-auto rounded-2xl border border-border bg-card/95 backdrop-blur-xl">
              {query.trim().length === 0 ? (
                ...
                <div className="p-2">
                  ...
                  {results.map((movie) => (
                    <button key={movie.id} onClick={onClose}
                      className="w-full flex items-center gap-3 px-3 py-3 rounded-xl hover:bg-secondary/60 transition-colors text-left"
                    >
                      {movie.image_url ? (
                        <img src={movie.image_url} alt={movie.title} className="w-10 h-14 rounded-lg object-cover shrink-0" />
                      ) : (
                        <div className="w-10 h-14 rounded-lg bg-secondary flex items-center justify-center shrink-0"><Film className="w-5 h-5 text-muted-foreground" /></div>
                      )}
